    ...
    , m_wayland(false)
{
    ...
    QDBusConnection::sessionBus().registerService(QStringLiteral("com.matsya.Session"));
    QDBusConnection::sessionBus().registerObject(QStringLiteral("/Session"), this);

    QCommandLineParser parser;
    parser.setApplicationDescription(QStringLiteral("Matsya Session"));
...
void Application::initEnvironments()
{
    ...
    // Environment
    qputenv("DESKTOP_SESSION", "Matsya");
    qputenv("XDG_CURRENT_DESKTOP", "Matsya");
    qputenv("XDG_SESSION_DESKTOP", "Matsya");

    // Qt
    qputenv("QT_QPA_PLATFORMTHEME", "matsya");
    qputenv("QT_PLATFORM_PLUGIN", "matsya");
...
{
    QSettings settings(QSettings::UserScope, "matsyaos", "language");
...
void Application::initXResource()
{
    ...
    // For matsya-wine
    qputenv("MATSYA_FONT_DPI", QByteArray::number(fontDpi));
...
void Application::initKWinConfig()
{
    ...
    settings.setValue("library", "org.matsya.decoration");
...
// But it won't work if matsyaDE is not started by systemd.
...
{
    // bool isMatsyaOS = QFile::exists("/etc/matsyaos");
